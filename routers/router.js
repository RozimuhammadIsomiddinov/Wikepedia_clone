...
app1
  .get("/:request", async (req, res) => {
    const finder = await File.find().select({
      senderName: 1,
      fileName: 1,
      _id: 0,
    });
  ...
  .post("/:paramData", async (req, res) => {
    ...
    try {
      ...
      if (findByDb.length == 0 && findByParam.length > 0) {
        res
          .status(201)
          .send(
      ...
      } else if (findByDb.length > 0 && findByDb[0].data === req.body.data) {
        return res
          .status(400)
          .send(
        ...
        return res
          .status(404)
          .send(
            "Siz so'ragan ma'lumot DataBaseda mavjud emas" +
              "\nSiz yuborgan malumot DataBasega saqlanmadi"
          );
      }
    } catch (err) {
      console.log(err);
